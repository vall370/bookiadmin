...
const sendingEmail =
  process.env.SENDING_EMAIL || "no-reply@mern-auth-server.herokuapp.com";
    ...
    User.findById(token._userId, function (err, user) {
      ...
      user.hashPassword().then(() =>
        ...
        user.save(function (err) {
          ...
          nodemailerMailgun.sendMail(mail).catch(() => {
...
router.post("/register", async (req, res) => {
  ...
  user = await User.findOne({ email: req.body.email.toLowerCase() }, function (
    err
  ) {
    if (err) {
      return res.status(500).send("An unexpected error occurred");
        ...
        token.save(function (err) {
          ...
          nodemailerMailgun
            ...
            .catch((error) => {
              return res
                .status(500)
                .json({ message: `Impossible to send email to ${user.email}` });
            });
        });
      }
    });
